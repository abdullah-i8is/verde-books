...
#     shutil.copy("main1.xlsx", "webapp/statements/"+str(month)+"-"+str(year)+".xlsx")
#     db,cursor=database()
...
#     result=cursor.fetchall()
#     print("result",result)
#     xfile = openpyxl.load_workbook("webapp/statements/"+str(month)+"-"+str(year)+".xlsx")
#     sheet = xfile['Sheet1']
...
#     titleFont=Font(size=9,name='Arial MT')
#     previousMonth=int(month)-1
#     temp=year
#     last_month = month - 1 if month > 1 else 12
#     last_year = year - 1 if month == 1 else year
#     last_day_previous_month = (datetime(year, month, 1) - timedelta(days=1)).day
#     last_day_current_month = (datetime(year, month + 1, 1) - timedelta(days=1)).day
#     last_month_date = datetime(last_year, last_month, last_day_previous_month).strftime('%b %d %Y')
#     current_month_date = datetime(year, month, last_day_current_month).strftime('%b %d %Y')
#     if month==1:
#        previousMonth=12
#        temp=int(year)-1

#     date_font = Font(size=9, name='Arial MT')
#     sheet['G13'].font = date_font
#     sheet['I13'].font = date_font
   
#     sheet['G13'] = last_month_date
#     sheet['I13'] = current_month_date
#     transCounter=1
#     emptyStatement=True
...
#      sheet.row_dimensions[cur].height = 13
#      sheet.column_dimensions['C'].width = 1
#      ddd = str(row[1])
#      try:
#         original_date = datetime.strptime(str(row[1]), "%Y-%m-%d")
...
#         except ValueError:
#           try:
#             original_date = datetime.strptime(str(row[1]), "%m/%d/%Y")
#             formatted_date = original_date.strftime("%m/%d/%Y")
#             ddd = formatted_date
#           except ValueError:
#             ddd = str(row[1])
...
#         sheet.merge_cells("B"+str(cur)+":"+"D"+str(cur))
#         sheet['C'+str(cur)].alignment = align
#         if str(row[3]) not in {'0.0', ''}:
#          try:
#              sheet['F' + str(cur)] = "{:.2f}".format(float(row[3]))
#          except ValueError:
#             sheet['F' + str(cur)] = row[3]
#         if str(row[4]) not in {'0.0', ''}:
#          try:
#              sheet['H' + str(cur)] = "{:.2f}".format(float(row[4]))
#          except ValueError:
#             sheet['H' + str(cur)] = row[4]  
...
#             cur=cur+3
#         if len(result)==3:
#             sheet['J'+str(34*int(statementPage))]=str(statementPage)+" of "+str(totalStatementPages)
#         else:
#             sheet['A'+str(cur)].alignment = Alignment(horizontal='left', vertical='top', wrap_text=True)
...
#     xfile.save("webapp/statements/"+str(month)+"-"+str(year)+str(temp)+".xlsx")
#     url="https://verdebooks.com/statements/"+str(month)+"-"+str(year)+str(temp)+".pdf"
#     try:
#         convertapi.api_secret = 'x94L5CeOZjoJ47VN'
#         result=convertapi.convert('pdf', {'File': 'webapp/statements/'+str(month)+'-'+str(year)+str(temp)+'.xlsx'}, from_format = 'xlsx')
#         result.file.save('webapp/statements/'+str(month)+"-"+str(year)+str(temp)+'.pdf')
...
def generateStatement(month,year):
    ...
    month=int(month)
    print(tempMonth)
    shutil.copy("main.xlsx", "./webapp/statements/"+str(month)+"-"+str(year)+".xlsx")
    db,cursor=database()
    query="select * from transactionalHistory where date LIKE '%"+str(year)+"-"+str(month)+"%' OR date LIKE '%"+str(year)+"-"+str(tempMonth)+"%' order by date,id"
    cursor.execute(query)
    result=cursor.fetchall()
    ...
    url="https://verdebooks.com/statements/"+str(month)+"-"+str(year)+str(temp)+".pdf"
    #convertapi.api_secret = '6dZbYsZNzn9w6d9d'
    try:
        convertapi.api_secret = 'XqfWIpvLatF0b12A'
        result=convertapi.convert('pdf', {'File': './webapp/statements/'+str(month)+'-'+str(year)+str(temp)+'.xlsx'}, from_format = 'xlsx')
        result.file.save('./webapp/statements/'+str(month)+"-"+str(year)+str(temp)+'.pdf')
    except:
        url="https://verdebooks.com/statements/"+str(month)+"-"+str(year)+str(temp)+".xlsx"
    return jsonify({"response":url})
